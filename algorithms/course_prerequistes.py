...
def topological_order(vertices, edges):
    # Initialize 2 adjacency lists based on the vertices given one that holds information about the children for each
    # node and other that holds count for the incoming edges on the nodes
    graph = {}
    inDegree = {}
    ...
    for edge in edges:
        ...
        inDegree[
            child] += 1  # Increment incoming edge for the child. Since there is an edge from parent to child the
        # child has dependency on parent.
    ...
    while len(sources):
        ...
        for child in graph[
            vertex]:  # Check the node's children one by one to; decrement count when you see it and check if they
            # can be added to the sources (when count becomes 0)
            inDegree[child] -= 1
